...
export default function PatientsPage() {
  const { getPatients, createPatient, updatePatient } = usePatients();
  ...
  async function handleSubmit(data: CreatePatientRequest | UpdatePatientRequest) {
    setSaving(true);
    try {
      if ("id" in data) await updatePatient(data);
  ...
  return (
    <ModulePage>
      ...
      <div className="mb-6 grid gap-4 md:grid-cols-4">
        ...
        <StatsCard title="Active Patients" value={stats.activePatients} />
        <StatsCard title="Today's Registrations" value={stats.todayRegistrations} />
      ...
      <div className="mt-6">
        <DataTable headers={["UHID","Patient Name","Phone","Gender","Age","Status","Actions"]}>
          {filtered.length===0 ? (
            <EmptyState
            ...
            <tr key={patient.id} className="border-b border-slate-800 last:border-none">
              <td className="px-4 py-3 text-sm text-slate-300">{patient.uhid}</td>
              <td className="px-4 py-3 text-white">{patient.firstName} {patient.lastName}</td>
              <td className="px-4 py-3 text-slate-300">{patient.phone}</td>
              <td className="px-4 py-3 text-slate-300">{patient.gender}</td>
              <td className="px-4 py-3 text-slate-300">{patient.age}</td>
              <td className="px-4 py-3 text-slate-300">{patient.status}</td>
